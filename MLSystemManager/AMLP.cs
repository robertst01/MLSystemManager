...
namespace MLSystemManager
{
	...
	class AMLP : SupervisedLearner
	{
		...
		public AMLP(Random rand, double rate, double momentum, string activation, string actParameter, int[] hidden)
		{
			m_rand = rand;
			m_rate = rate;
			m_momentum = momentum;
		...
		public override void VTrain(VMatrix features, VMatrix labels, double[] colMin, double[] colMax)
		{
			...
			for (int round = 1; round <= m_hidden.Length; round++)
			{
				if (round > 1)
				{
					// add the input nodes to the current output layer
					for (var i = 0; i < features.Cols(); i++)
					{
						m_layers[m_layers.Count - 1].Add(new InputNode(i, 0, colMin[i], colMax[i], m_rand));
					}
				}
				...
				for (var n = 0; n < m_hidden[round - 1]; n++)
				{
